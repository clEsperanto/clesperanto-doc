...
.. warning::
    ...
    Or install a different backend (e.g. CUDA) if your device supports it, see :doc:`installation instructions <install>` for more details.


Backends and devices
--------------------

By default, `pyclesperanto` uses the `OpenCL` as a backend which is automatically selected at import time along witht the last available device on your system.
...
Once a backend is selected, you can query the available devices with `cle.list_available_devices()` or get a more detailed list of your devices wiht `cle.info()`. 
...
By default, `pyclesperanto` automatically selects the last available device at import time, but the device order in a system is not guaranteed or the selected device may not be the one you want to use.
...
We can use the output of one operation as the input of the next operation, chaining the operations together.
The use of data transfer functions like ``push`` and ``pull`` is only necessary at the beginning and end of the pipeline, once we are done working on the GPU. 

.. note::

    Memory transfert being one of the most costly operations in terms of time, it is good practice to minimize them by keeping data on the GPU as long as possible and only transferring it back to the host when necessary.

Element-wise arithmetic optimization
------------------------------------

`pyclesperanto` provides arithmetic operations like addition, subtraction, multiplication, or division between arrays in a simple way:
...
In addition of arithmetic operators (`+`, `-`, `*`, `/`), the `evaluate()` function also supports mathematical functions like `sqrt()`, `exp()`, `log()`, etc.
...
Making them work together can be challenging and in this regard most libraries follow numpy ecosystem standards to ensure compatibility and interoperability.
This is also the case for `pyclesperanto` which aims to provide seamless interoperability with other libraries when it is possible.

Working with other CPU libraries
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

`pyclesperanto` being designed to work on GPU devices, interoperability with CPU-only libraries cannot be copy-free.
A memory transfer from the GPU to the host or reverse is required to exchange data between `pyclesperanto` and CPU libraries.
...
.. note::
    ...
    As this interoperability is not copy-free, it will trigger a memory transfer from the device to the host or reverse, which can be costly in terms of time.

.. note::

    `cle.asarray()` is equivalent to `cle.push()` and `np.asarray()` on a clesperanto array is equivalent to `cle.pull()`.


Working with other GPU libraries
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Other libraries may also support GPU acceleration and the question of interoperability between GPU libraries become more complex.
By default, it is always possible to exchange data between GPU libraries through the host memory, similarly to CPU libraries as described in the previous section, but this is not optimal as it involves costly memory transfers.
In specific cases, it is possible to exchange data without memory transfer, which is known as copy-free interoperability.

In pyclesperanto, such copy-free interoperability is possible with other libraries supporting the `DLPack protocol <https://dmlc.github.io/dlpack/latest/index.html>`_ and using the same GPU backend language (e.g., `CUDA`).
A example of copy-free interoperability between `pyclesperanto` and `CuPy` is shown below.
...
.. code:: python
    ...
    # Apply image processing operationswith pyclesperanto
